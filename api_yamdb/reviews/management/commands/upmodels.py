...
from reviews.models import Category, Comment, Genres, GenreTitle, Review, Title
...
class Command(BaseCommand):
    ...
    def users_update():
        with open(
            os.path.join(BASE_DIR, 'static/data/users.csv'), encoding='utf-8'
        ) as csv_file:
    ...
    def title_update():
        ...
        with open(
            os.path.join(BASE_DIR, 'static/data/titles.csv'), encoding='utf-8'
        ) as csv_file:
            csv_reader = csv.DictReader(csv_file, delimiter=',')
            for row in csv_reader:
                id = row['id']
    ...
    def genre_update():
        ...
        with open(
            os.path.join(BASE_DIR, 'static/data/genre.csv'), encoding='utf-8'
        ) as csv_file:
            csv_reader = csv.DictReader(csv_file, delimiter=',')
            for row in csv_reader:
                id = row['id']
    ...
    def review_update():
        with open(
            os.path.join(BASE_DIR, 'static/data/review.csv'), encoding='utf-8'
        ) as csv_file:
            csv_reader = csv.DictReader(csv_file, delimiter=',')
            for row in csv_reader:
                id = row['id']
